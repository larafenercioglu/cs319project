...
public class Class {
    ...
    public boolean formRandomGroups(List<Group> groups, Map<Integer,List<Student>> groupStudents, List<Student> students, Class c){
        ...
        System.out.println("---inside random groups map is: "+ groupStudents +"---");
        //iterate over groups
        for (int i = 0; i < groups.size(); i++) {
            // if groups size less than half maxsize
            if ( groups.get(i).getStudentIdList().size() <= c.getProject().getMaxGroupSize() / 2 && flag == false ) {
                indexOfGroupToReform = i;
                System.out.println("group Id: "+indexOfGroupToReform+" groups is -->"+groups.get(indexOfGroupToReform)+" forming...");
                flag = true;
            }
            // if there is another one
                ...
                List <Student> studentArr = groupStudents.get(groups.get(indexOfGroupToReform).getGroupId());// a
                id = groups.get(i).getGroupId();
                System.out.println("adding group " + studentArr +" to group "+groupStudents.get(id));
                //add students from one group one by one to the another one
                System.out.println("listSize is: " + listSize+" "+i+"group size is "+ groupStudents.get(id).size());
                for( int t = 0 ; t < listSize ; t++ ) {
                    ...
                    //groups[indexOfGroupToReform] += ith group with student id t
                    //System.out.println("Map TEST BEFORE STUDENT "+groupStudents.get(i) +": "+groupStudents.get(i));
                    //System.out.println("Map student "+groupStudents.get(i) +": "+groupStudents.get(i).get(t));
                    //System.out.println("groups.get(indexOfGroupToReform) is :" + groups.get(indexOfGroupToReform));
                    //System.out.println("groupStudents.get(i).get(t) is " + groupStudents.get(id).get(t));
                    groups.get(indexOfGroupToReform).addGroupMember(groupStudents.get(id).get(t));
                    studentArr.add(groupStudents.get(id).get(t));
                    //System.out.println("Studentarr in loop : " + studentArr);
                }
                //update groupStudents map
                //System.out.println("removing group: "+i);
                groupStudents.remove(id);
                //System.out.println("AFTER REMOVING GROUP FROM MAP: "+groupStudents);
                // update groups array
                groups.remove(i);
                //groupStudents[indexOfGroupToReform] = studentArr
                //System.out.println("BEFORE NEW GROUP MAP --> GROUP IS: " + groups);
                //groupStudents.put(indexOfGroupToReform,studentArr);
                //System.out.println("NEW GROUP FROM MAP: "+groupStudents);
        ...
        System.out.println("groupStudents" + groupStudents);
        System.out.println("students" + students);
        System.out.println("class " + c.getGroupIdList());
        return true;
